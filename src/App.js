...
class App extends Component {
  state = {king: tabsList[0].tabId}
  onClickBalu = tabId => {
    this.setState({king: tabId})
  }
  onGetPics = () => {
    const {king} = this.state
    const h = imagesList.filter(eachItem => eachItem.category === king)

    return h
  }
  render() {
    const b = this.onGetPics()
    console.log(b)
    return (
      <div>
        <ul>
          {tabsList.map(eachItem => (
            <p onClick={this.onClickBalu}>{eachItem.displayText}</p>
          ))}
        </ul>
        <ul>
          {b.map(eachItem => (
            <img src={eachItem.thumbnailUrl} />
          ))}
        </ul>
      </div>
    )
